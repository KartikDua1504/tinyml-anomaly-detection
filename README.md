...
Data was captured using an ESP32 and MPU6050 3-axis accelerometer taped to a ceiling fan. Each sample is about 200 samples of all 3 axes captured over the course of 1 second. Fan was run at multiple speeds (off, low, medium, high) with and without a weight. 1 "weight" is one US quarter taped to one of the fan's blades to create an offset motion. All raw data is stored in the ceiling-fan-dataset directory.
...
1. Terminal-Centric Development Environment

To streamline development, we migrated the project to a modern CLI workflow:

    Editor: Helix and Neovim for code editing and configuration.

    Build Tool: PlatformIO Core (CLI) for managing ESP32 builds and library dependencies without the Arduino IDE.

    Environment: Managed via Python Virtual Environments (venv) to isolate TensorFlow and PlatformIO dependencies.

2. Modern Firmware Structure (PlatformIO)

We restructured the deployment code into a PlatformIO project for better management of the TensorFlow Lite Micro (TFLM) libraries.

Project Setup:
Bash

# Initialize a new firmware project
mkdir firmware_sim && cd firmware_sim
pio project init --board esp32dev

Configuration (platformio.ini): We utilized the community-maintained TensorFlow library for ESP32 compatibility:
Ini, TOML
...
3. Rapid Prototyping with Edge Impulse

While the original repo focuses on manual training in Jupyter, we integrated Edge Impulse to provide a "Live Simulation" mode using a smartphone's internal accelerometer.

The Edge Impulse Workflow:

    Mobile Client: Used the Edge Impulse mobile browser client to turn a smartphone into a wireless vibration sensor.

    Data Acquisition: * Captured "OKAY" data (phone stationary on a desk).

        Captured "ANOMALY" data (simulating fan failure by shaking/vibrating the phone).

    Spectral Analysis: Used Digital Signal Processing (DSP) to extract frequency features from the raw accelerometer data.

    K-Means Anomaly Detection: Trained a clustering model that defines a "Safe Zone" around normal data. Any vibration falling outside this cluster is flagged as an anomaly.

4. Hardware Simulation (Wokwi Integration)

For users without immediate access to physical hardware, we developed a simulation path using Wokwi:

    Virtual Components: Simulated an ESP32 connected to an MPU6050 accelerometer via I2C.

    Diagram Configuration: Custom diagram.json to map the SDA/SCL pins (GPIO 21/22) for the sensor.

    Validation: Verified the C++ model inference by manually "shaking" the virtual sensor in the browser to trigger MSE (Mean Squared Error) spikes.

5. Compiled Model Deployment

We converted the Python-trained .h5 models into C++ headers (model.h) to be compiled directly into the ESP32 firmware.

    Variable Name Alignment: Ensured the const unsigned char array name in the exported header matched the TFLM interpreter initialization in main.cpp.

    Inference Loop: The firmware collects 128 samples (384 data points), runs a single inference pass, and calculates the MSE to determine the anomaly status in real-time.
...
grep "unsigned char" src/model.h
